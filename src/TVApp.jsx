...
function LiveTVPanel({ onPlay, channels, loading }) {
  ...
  return (
    <div className="px-6 md:px-8 pb-10 pt-6">
      <div className="text-lg font-black mb-1" style={{ color: C.text, fontFamily: "'Space Grotesk', sans-serif" }}>Live TV</div>
      <p className="text-xs mb-5" style={{ color: C.textFaint }}>
        {loading ? "Loading channels\u2026" : "Channels with a connected stream are playable; others are listed but offline."}
      </p>
      {!loading && channels.length === 0 ? (
        <div className="text-sm py-10 text-center" style={{ color: C.textFaint }}>No channels available yet.</div>
      ) : (
        <>
    ...
    <div className="px-6 md:px-8 pb-10 pt-6">
      ...
      {items.length === 0 ? (
        <div className="flex flex-col items-center justify-center gap-3 py-16 text-center">
          ...
          <div className="text-sm" style={{ color: C.textDim }}>Nothing here yet.</div>
          <div className="text-xs" style={{ color: C.textFaint }}>Tap the heart icon on any title to add it to your favorites.</div>
        </div>
      ) : (
...
function SettingsPage({ onLogout }) {
  ...
  return (
    <div className="px-6 md:px-8 pb-10 pt-6 max-w-xl">
      <div className="text-lg font-black mb-5" style={{ color: C.text, fontFamily: "'Space Grotesk', sans-serif" }}>Settings</div>

      <div className="rounded-xl p-4 mb-5" style={{ background: C.bgCard, border: `1px solid ${C.line}` }}>
        <ToggleRow icon={Bell} label="Push Notifications" sub="Get notified about new episodes and billing" on={pushNotif} onChange={setPushNotif} />
        <ToggleRow icon={Play} label="Autoplay Next Episode" sub="Automatically continue to the next episode" on={autoplay} onChange={setAutoplay} />
        <ToggleRow icon={Volume2} label="Data Saver Mode" sub="Reduce streaming quality on mobile data" on={dataSaver} onChange={setDataSaver} />
        <div className="flex items-center justify-between py-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="text-sm font-medium text-white">Parental Controls</div>
              <div className="text-xs" style={{ color: C.textFaint }}>Restrict mature content</div>
            </div>
          </div>
          <button
      ...
      <div className="rounded-xl p-4 mb-5" style={{ background: C.bgCard, border: `1px solid ${C.line}` }}>
        ...
        <div className="flex gap-2 flex-wrap">
          {["English", "French", "Krey\u00f2l", "Espa\u00f1ol"].map((l) => (
            <button
              key={l}
              onClick={() => setLang(l)}
              className="px-3 py-1.5 rounded-full text-xs font-semibold"
              style={{
                background: lang === l ? C.primary : C.bgRaised,
                color: lang === l ? "#fff" : C.textDim,
                border: `1px solid ${lang === l ? C.primary : C.line}`,
              }}
            >
              {l}
            </button>
      ...
      <div className="rounded-xl p-4 mb-5 flex items-center gap-3" style={{ background: C.bgCard, border: `1px solid ${C.line}` }}>
        ...
        <div className="text-xs" style={{ color: C.textFaint }}>BenTech TV Stick — App version 1.0.0</div>
      </div>
  ...
  return (
    <div className="absolute right-0 top-12 w-80 rounded-xl overflow-hidden z-30" style={{ background: C.bgCard, border: `1px solid ${C.line}`, boxShadow: "0 12px 30px rgba(0,0,0,.5)" }}>
      <div className="flex items-center justify-between px-4 py-3" style={{ borderBottom: `1px solid ${C.line}` }}>
        ...
        <button onClick={onMarkAllRead} className="text-xs font-semibold" style={{ color: C.cyan }}>Mark all read</button>
      </div>
  ...
  return (
    <div className="absolute right-0 top-12 w-56 rounded-xl overflow-hidden z-30" style={{ background: C.bgCard, border: `1px solid ${C.line}`, boxShadow: "0 12px 30px rgba(0,0,0,.5)" }}>
      <div className="px-4 py-3" style={{ borderBottom: `1px solid ${C.line}` }}>
        ...
        <div className="text-xs" style={{ color: C.textFaint }}>Customer</div>
      </div>
      <button onClick={onSettings} className="w-full flex items-center justify-between px-4 py-3 text-sm text-white" style={{ borderBottom: `1px solid ${C.line}` }}>
        Settings <ChevronRight size={14} color={C.textFaint} />
      </button>
    ...
    <div className="w-full h-screen flex" style={{ background: C.bg, fontFamily: "'Inter', sans-serif" }}>
      ...
      <div className="flex-1 overflow-y-auto" onClick={() => closePopovers()}>
        ...
        <div className="flex items-center justify-between px-6 py-5 md:px-8 relative">
          ...
          {searchOpen ? (
            <div className="flex items-center gap-2 flex-1 max-w-md" onClick={(e) => e.stopPropagation()}>
              ...
              <button onClick={() => { setSearchOpen(false); setQuery(""); }} className="text-xs font-semibold" style={{ color: C.textDim }}>Cancel</button>
            </div>
          ) : (
            <div className="hidden md:block text-lg font-semibold" style={{ color: C.text, fontFamily: "'Space Grotesk', sans-serif" }}>
              {{ home: "Good morning", live: "Live TV", movies: "Movies", series: "Series", fav: "Favorites", settings: "Settings" }[active]}
            </div>
          )}
        ...
        {searchOpen && query.trim() && (
          <div className="px-6 md:px-8 pb-8">
            ...
            {searchResults.length === 0 ? (
              <div className="text-sm" style={{ color: C.textDim }}>No matches found.</div>
            ) : (
              <div className="grid gap-4" style={{ gridTemplateColumns: "repeat(auto-fill, minmax(140px, 1fr))" }}>
        ...
        {!searchOpen && active === "movies" && (
          <GridPage title="Movies" subtitle="Thousands of movies to explore" items={movies} onPlay={setPlaying} favorites={favorites} onToggleFav={toggleFav} />
        )}
        {!searchOpen && active === "series" && (
          <GridPage title="Series" subtitle="Popular TV shows" items={series} onPlay={setPlaying} favorites={favorites} onToggleFav={toggleFav} />
        )}
        {!searchOpen && active === "fav" && (
          <GridPage title="Favorites" subtitle="Your saved movies and series" items={favItems} onPlay={setPlaying} favorites={favorites} onToggleFav={toggleFav} />
        )}
        ...
        {!searchOpen && active === "home" && (
          <div className="px-6 md:px-8 pb-10 space-y-8">
              ...
              <div className="relative flex flex-col md:flex-row items-center justify-between gap-6 px-6 py-8 md:px-10 md:py-10">
                <div>
                  ...
                  <div className="mt-4 space-y-1.5 text-sm" style={{ color: C.textDim }}>
                    {["Stable Connection", "High Speed", "24/7 Support"].map((f) => (
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              {[
                { label: "LIVE TV", sub: "Watch live channels", icon: Tv, badge: "LIVE", grad: [C.ember, "#5E0F1E"], id: "live" },
                { label: "MOVIES", sub: "Thousands of movies", icon: Film, grad: [C.purple, "#3A1E7A"], id: "movies" },
                { label: "SERIES", sub: "Popular TV shows", icon: Layers, grad: ["#1E3A5F", "#0B1830"], id: "series" },
                { label: "FAVORITES", sub: "Your favorite content", icon: Heart, grad: [C.amber, "#8A5A0E"], id: "fav" },
              ].map((tile) => (
                <div
                  key={tile.label}
            ...
            {/* Continue watching */}
            <div>
              <div className="text-sm font-bold tracking-wide mb-3" style={{ color: C.text }}>CONTINUE WATCHING</div>
            ...
            {/* Trending row */}
            <div>
              <div className="text-sm font-bold tracking-wide mb-3" style={{ color: C.text }}>TRENDING NOW</div>
              <div className="flex gap-4 overflow-x-auto pb-2">
...
function LiveEventsSection({ onPlay }) {
  ...
  return (
    <div>
      <div className="text-sm font-bold tracking-wide mb-3" style={{ color: C.text }}>LIVE & UPCOMING EVENTS</div>
      <div className="flex gap-4 overflow-x-auto pb-2">
      ...
      <div className="relative w-full max-w-sm px-6">
        ...
        <div className="rounded-2xl p-6" style={{ background: C.bgCard, border: `1px solid ${C.line}` }}>
          ...
          <div className="text-center text-xs mt-5" style={{ color: C.textFaint }}>
            Don't have a code? <span style={{ color: C.cyan }}>Contact your provider</span>
          </div>
        </div>
        <div className="rounded-xl p-4 mt-4" style={{ background: C.bgCard, border: `1px solid ${C.line}` }}>
          <div className="text-xs font-semibold mb-2" style={{ color: C.textDim }}>Need help?</div>
